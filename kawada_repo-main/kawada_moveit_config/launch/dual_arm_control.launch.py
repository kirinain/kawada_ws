...
def generate_launch_description():
    ...
    moveit_controllers = {
        ...
        'moveit_simple_controller_manager.controller_names': controllers_yaml.get('controller_names', ['right_arm_controller', 'left_arm_controller']),
    }
    ...
    # 8. Joint State Publisher GUI for manual joint control
    ...
    # 9. Cartesian Control Nodes
    right_arm_cartesian_control = Node(
        ...
        parameters=[
            ...
            {'robot_description_kinematics': kinematics_config},
            {
                ...
                'use_gazebo': True,
                'cartesian_command_in_topic': '/right_arm/cartesian_command',
                'joint_topic': '/joint_states',
                'status_topic': '/right_arm/status',
                'use_sim_time': True,
            }
        ],
    )

    left_arm_cartesian_control = Node(
        package="moveit_servo",
        executable="servo_node",
        name="left_arm_servo",
        output="screen",
        parameters=[
            ...
            {'robot_description_kinematics': kinematics_config},
            {
                ...
                'use_gazebo': True,
                'cartesian_command_in_topic': '/left_arm/cartesian_command',
                'joint_topic': '/joint_states',
                'status_topic': '/left_arm/status',
                'use_sim_time': True,
            }
        ],
    )

    # 10. Simple Cartesian Control Publisher (for testing)
    right_arm_cartesian_publisher = Node(
        package="kawada_moveit_config",
        executable="cartesian_control_publisher.py",
        name="right_arm_cartesian_publisher",
        output="screen",
        parameters=[{'use_sim_time': True}],
    )

    left_arm_cartesian_publisher = Node(
        package="kawada_moveit_config", 
        executable="cartesian_control_publisher.py",
        name="left_arm_cartesian_publisher",
        output="screen",
        parameters=[{'use_sim_time': True}],
    )
    ...
    delayed_moveit = TimerAction(period=10.0, actions=[move_group_node, rviz_node, joint_state_publisher_gui])
    delayed_servo = TimerAction(period=12.0, actions=[right_arm_cartesian_control, left_arm_cartesian_control])
    delayed_publishers = TimerAction(period=14.0, actions=[right_arm_cartesian_publisher, left_arm_cartesian_publisher])
